...
const AUDIO_ONLY = process.argv.includes('--audio-only');   // 既存の build/video_noaudio.mp4 を使う
const FPS = QUICK ? 12 : 30;
const GAP = 0.7;             // シーン間の間
const W = 1280, H = 720;
const OUT = 'build/jimakuChan_v2_intro.mp4';
...
// 3) BGM
log('BGM 合成…');
execSync(`python3 bgm.py ${TOTAL.toFixed(2)} build/bgm.wav`, { stdio: 'inherit' });
...
const ff = spawn('ffmpeg', ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', String(FPS), '-i', '-', '-c:v', 'libx264', '-preset', 'medium', '-crf', '18', '-pix_fmt', 'yuv420p', '-r', String(FPS), 'build/video_noaudio.mp4']);
...
const inputs = ['-i', 'build/video_noaudio.mp4', '-i', 'build/bgm.wav'];
